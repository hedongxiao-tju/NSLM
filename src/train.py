...
try:
    ...
    from .plm_checkers import RobertaChecker
...
mAutoModel = {
    ...
    'hfl/chinese-roberta-wwm-ext-large': RobertaChecker,
}
    ...
    for epo in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "img_context_input_ids_list": batch[2],
                "img_context_attention_mask_list": batch[3],
                "img_input": batch[-2],
                "labels": batch[-1],
            }

            outputs = model(**inputs)
            loss, _loss2, logits = outputs[0], outputs[1], outputs[
                2]

            loss2, loss3 = _loss2  #  loss2:_elbo loss; loss3 : logic_loss


            label_truth += batch[-1].tolist()
            y_predicted += outputs[-2].tolist()
            z_predicted += outputs[-1].tolist()
            ...
            a_loss2 += loss2.detach().cpu().numpy() * args.gradient_accumulation_steps
            a_loss3 += loss3.detach().cpu().numpy() * args.gradient_accumulation_steps

            y = np.argmax(y_predicted, axis=-1).tolist()
            outputs, results, y_zh, z_list, y_zs, y_zs_logit = compute_metrics(label_truth, y, z_predicted)

            all_accuracy += np.mean(
                inputs["labels"].detach().cpu().numpy() == logits.detach().cpu().numpy().argmax(axis=-1)
            )
            description = "EPOCH:{:>2}, Global step: {:>4}, Loss: {:>.6f}, loss_elbo: {:>.6f}, loss_logic: {:>.6f}, Acc: {:>.6f},Acc2: {:>.4f},Accy_zh: {:>.4f},Accy_zs: {:>.4f}, Agreh: {:>.4f}, Agres: {:>.4f}".format(
                epo + 1,
                global_step,
                all_loss / (step + 1),
                a_loss2 / (step + 1),
                ...
                results['z_Acc_hard'],
                results['z_Acc_soft'],
                results['Agreement_hard'],
                results['Agreement_soft'],
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        for key, value in results.items():
                            logger.warning(f"Step: {global_step}, test_{key}: {value}")
                            wdblogger.log_metrics({"test_{}".format(key): value}, global_step)
...
def evaluate(args, data_processor, model, tokenizer, prefix=""):
    ...
    dataset = data_processor.load_and_cache_data("test", tokenizer, args.data_tag)
    test_sampler = SequentialSampler(dataset) if args.local_rank == -1 else DistributedSampler(dataset)
    ...
    label_truth, y, y_predicted, z_predicted = \
        do_testuate(tqdm(test_dataloader), model, args, during_training=True, with_label=False)
...
def do_evaluate(dataloader, model, args, during_training=False, with_label=True):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            outputs = model(**inputs)  # (loss, (neg_elbo, logic_loss), y, z_softmax)

            if during_training and (i < 3 and (args.logic_lambda != 0)):
                logger.warning(f'* all loss:\n {outputs[0][:5]}\n')
                logger.warning(f'* loss elbo:\n {outputs[1][:5]}\n')
                logger.warning(f'* loss logic:\n {outputs[2][:5]}\n')
                logger.warning(f'* predicted y:\n {outputs[-2][:5]}\n')
                logger.warning(f'* z Logic outputs:\n {outputs[-1][0][:5]}.\n real Labels: {batch[-1][:5]}\n')
    ...
    y = np.argmax(y_predicted, axis=-1).tolist()  # 0/1

    return label_truth, y, y_predicted, z_predicted
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="The maximum total input text sequence length after tokenization. "
    ...
    parser.add_argument('--prior', default='rand', type=str, choices=['rand'],
    ...
    parser.add_argument(
        "--per_gpu_test_batch_size", default=8, type=int, help="Batch size per GPU/CPU for testuation.",
    )
    parser.add_argument(
        "--hs", default=256, type=int, help="self.myhs", )

    parser.add_argument(
        "--share_hs", default=128, type=int, help="self.share_hs", )
    ...
    data_processor = DataProcessor(
        ...
        args.max_seq1_length,  # text
        args.max_seq2_length,  # o_text
